...
Demo	Demo	UCSC	S%s		S%sa		Normal - blood derived	S%sa1		upload	uploader	1.0.0	fastq.gz	ERR030886_1.fastq.gz
Demo	Demo	UCSC	S%s		S%sa		Normal - blood derived	S%sa1		upload	uploader	1.0.0	fastq.gz	ERR030886_2.fastq.gz''' % (str(specimen), str(specimen), str(specimen))
        f = open('sample.tsv', 'w')
        print >>f, template
        f.close()
        # execute upload
